  ...
  try {
    ...
    for (let i = 0; i < saleData.items.length; i++) {
      ...
      if (stock.CANTIDAD_SUCURSAL < item.cantidad) {
        throw new Error(
          `Stock insuficiente para el producto ID ${item.idProducto}. Stock disponible: ${stock.CANTIDAD_SUCURSAL}, Solicitado: ${item.cantidad}`
        );
      }
    }
    ...
    if (cashError.message.includes("No hay caja abierta")) {
      throw new Error(
        "No se puede realizar la venta. Debe abrir una caja antes de realizar ventas."
      );
    }
    throw cashError;
  }
